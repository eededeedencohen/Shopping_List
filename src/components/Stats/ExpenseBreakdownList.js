...
import styles from "./ExpenseBreakdownList.module.css";
...
const ExpenseBreakdownList = ({ data, selectedCategory }) => {
  ...
  return (
    <div className={styles['expense-breakdown-list']}>
      <h2>פירוט מוצרים</h2>
      <ul>
        {products.map((product, index) => (
          <div key={index} className={styles['ebl_product_details']}>
            <div className={styles['ebl_total_price']}>₪{product.totalPrice.toFixed(2)}</div>
            <div className={styles['ebl_supermarket']}>רמי לוי שיווק השקמה</div>
            <div className={styles['ebl_amount']}>{product.amount}X</div>
            <div className={styles['ebl_product']}>
              <div className={styles['ebl_product_name']}>{product.name}</div>
              <div className={styles['ebl_product_brand']}>{product.brand}</div>
            </div>
            <div className={styles['ebl_category_logo']}>
              <img
                src={categoryIcons[product.category] || otherLogo}
                alt={`${product.category} Logo`}
                className={styles['ebl_logo_image']}
              />
            </div>
          </div>
        ))}
